...
generate
genvar i;
  for(i = 0;i<k*k;i=i+1) // For every element of the kernel
  begin: MAC
    if((i+1) % k ==0)                      //end of a row in the kernel
    begin
      if(i==k*k-1)                        // end of the entire kernel
      begin
        mac_manual #(.N(N)) mac(                     //implements a*b+c
            ...
            // Output is directly suppied to overall output
            );
      end
      else
      begin
        
        wire [N-1:0] tmp2; // shift_reg_new
